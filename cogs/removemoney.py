...
class Remove(commands.Cog):
    ...
    async def removemoney(self, ctx, arg1, arg2):
        if ctx.author.id == self.collection.find_one("settings")["owner_id"]:
            bal = self.collection.users.find_one({"id": int(arg1)})["cash"]
            self.collection.users.update_one({"id": int(arg1)}, {'$set': {"cash": bal - int(arg2)}})
