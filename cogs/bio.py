...
class Bio(commands.Cog):
    ...
    async def bio(self, ctx, member:discord.Member = None):
        if member is None:
            emb = discord.Embed(title=f"Биография {ctx.author}", description=self.collection.users.find_one({"id": ctx.author.id})["bio"], colour=discord.Colour.blue())
            emb.set_footer(text=ctx.author, icon_url=ctx.author.avatar_url)
            await ctx.send(embed=emb)
        else:
            emb = discord.Embed(title=f"Биография {member}", description=self.collection.users.find_one({"id": member.id})["bio"], colour=discord.Colour.blue())
            emb.set_footer(text=ctx.author, icon_url=ctx.author.avatar_url)
            await ctx.send(embed=emb)
